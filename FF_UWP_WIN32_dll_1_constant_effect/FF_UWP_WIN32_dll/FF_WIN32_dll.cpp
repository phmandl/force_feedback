...
ForceFeedback::ConstantForceEffect^ FF_constant;
...
int initForceFeedback(int samplingTime) {
    FF_constant = ref new ForceFeedback::ConstantForceEffect(); 

    IAsyncOperation<ForceFeedbackLoadEffectResult>^ request2 = wheel->WheelMotor->LoadEffectAsync(FF_constant);

    int error = 0;

    auto task2 = create_task(request2);
    task2.then([&error,&samplingTime](ForceFeedbackLoadEffectResult result) {
        if (ForceFeedbackLoadEffectResult::Succeeded == result)
        {
            TimeSpan time;
            time.Duration = samplingTime; // samplingtime is in 100 ns Ticks --> Ts*1e7 Ticks
            FF_constant->SetParameters(Windows::Foundation::Numerics::float3(0.0f, 0.0f, 0.0f), time);
            FF_constant->Start();
        }
        else
        {
...
void FF_constant_gain(float powerGain, int samplingTime) {
    // now start the new one
    //FF_constant->Gain = powerGain;
    TimeSpan time;
    time.Duration = samplingTime; // samplingtime is in 100 ns Ticks --> Ts*1e7 Ticks
    FF_constant->SetParameters(Windows::Foundation::Numerics::float3(powerGain, 0.0f, 0.0f), time);
}
